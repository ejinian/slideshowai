...
**The CTA must serve the composer Goal.** The Goal pill text arrives appended to the prompt ("Goal of this post: Grow followers.") and the model half-ignored it (~50% of CTAs had no follow-ask). Both `buildUser`s now instruct the CTA to serve the stated goal explicitly.
...
**AI image generation was REMOVED (2026-07-17).** The `gpt-image-1` fallback (`lib/generate/aiImage.ts`), the "AI" Source option, and the `"auto"` background mode are all gone (MVP scope). The composer's source is now a binary toggle: **Upload** (default; the user's own photos, image-first) ↔ **stock** (live Pexels). Don't reintroduce AI-gen without asking.
