...
- **Auth** `e2e/auth.setup.ts`: provisions a reusable onboarded test user `e2e@slideshowai.test` + a fake `tiktok_connections` row via `SUPABASE_SECRET_KEY` (admin), logs in through the landing page's LoginModal (`/?auth=login` — the standalone `/login`/`/signup` pages were removed 2026-07-22; they now redirect to `/?auth=…` which auto-opens the matching modal, and sign-out lands on `/`), saves `storageState`. Touches your Supabase (never OpenAI).
